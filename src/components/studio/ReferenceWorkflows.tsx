...
const platformTasks = [['Identify the problem','Outline the workflow','Confirm success criteria'],['Map the user journey','Review the page structure','Agree access by role'],['Identify approved sources','Map fields and permissions','Validate the integration'],['Test normal cases','Review exceptions','Check the release criteria'],['Confirm operating owners','Agree monitoring and support','Document rollback steps']];
const agentContexts = ['New starter onboarding','Order coordination request','Account reconciliation request','Recurring team request'];
...
export function ReferenceWorkflow({serviceKey}:{serviceKey:ReferenceServiceKey}) {
  ...
  return <section className={`rp-light rp-section xr-workflow xr-${data.art}`} id="workflow"><div className="container">
    ...
    <div className="xr-example"><p className="xr-example-label">Illustrative workflow</p>
      {isKnowledge?<><form className="xr-search" onSubmit={e=>{e.preventDefault();setAsked(true);setStep(supported?3:4);}}><Search size={18}/><input aria-label="Ask a sample knowledge question" value={question} onChange={e=>{setQuestion(e.target.value);setAsked(false);}} required maxLength={250}/><button type="submit">Ask</button></form><div className="xr-knowledge-body"><div className="xr-knowledge-stages">{data.steps.slice(1).map((s,i)=><div key={s}><Circle size={15}/><span>{s}<small>{asked?(supported?(i<3?'Complete':'No gaps in this example'):'Insufficient evidence'):(i===0?'Searching your allowed documents…':'Pending')}</small></span></div>)}</div><div className="xr-source"><FileText size={31}/><h3>{current.source.split(' · ')[0]}</h3><div className="xr-source-lines"/><p>Sample source</p></div></div>{asked&&<div className="xr-answer" aria-live="polite">{supported?<><p>{current.answer}</p><details><summary>View source: {current.source}</summary><blockquote>{current.quote}</blockquote></details></>:<p>This example has no source for that question. Try “{current.question}” to see a source-supported response.</p>}</div>}</>
      :isAgent?<><p className="xr-sample-context">Example: {agentContexts[tab]}</p><div className="xr-agent-progress">{data.steps.map((s,i)=><div key={s} className={step===i?'active':''}><span>{i<step?<Check size={15}/>:<Circle size={15}/>}</span><p>{s}</p><small>{i===4&&step===4&&!approved?'Awaiting approval':i<step?'Complete':i===step?'In progress':'Not started'}</small></div>)}</div>{step>=3&&<div className="xr-demo-approval" aria-live="polite">{approved?<p>Example approved. The action can be recorded.</p>:<><p>This example pauses before a commitment.</p><button type="button" onClick={()=>{setApproved(true);setStep(4);}}>Approve example <Check size={16}/></button></>}</div>}</>
      :<div className="xr-platform"><aside><strong>BYBO</strong>{['Workflow','Data','Users','Settings'].map(x=><span key={x}>{x}</span>)}</aside><div className="xr-platform-task"><h3><span>{step+1}</span>{data.steps[step]}</h3>{platformTasks[step].map((task,i)=>{const id=`${tab}-${step}-${i}`;return <label key={task}><input type="checkbox" checked={checked.includes(id)} onChange={e=>setChecked(prev=>e.target.checked?[...prev,id]:prev.filter(x=>x!==id))}/>{task}</label>;})}<button type="button" onClick={()=>setStep(s=>(s+1)%5)}>{step===4?'Restart example':'Next'}<ArrowRight size={14}/></button></div></div>}
