...
const steps = ['Receive the file','Read the details','Check your rules','Review exceptions','Update the record'];
...
export function DocumentDemo() {
  ...
  return <section className="rp-light rp-section rp-demo" id="workflow"><div className="container">
    ...
    <div><div className="rp-workbench"><header><strong>Illustrative workflow</strong><span aria-hidden="true">•••</span></header><div className="rp-workbench-body">
      <div className="rp-stage-list" aria-hidden="true">{['Incoming','Reading','Checking','Exceptions','Update'].map((label,i)=><div key={label} className={step===i?'active':''}><Folder size={15}/>{label}{i<step&&<CheckCircle2 size={12}/>}</div>)}</div>
      <div className="rp-example-file"><strong>{example.title}</strong><small>{example.reference} · Sample</small><div className="rp-file-rule"/><div className="rp-file-table"><span>Description</span><span>Qty</span><span>Amount</span>{['Service / item A','Service / item B','Service / item C'].map((label,i)=><div key={label}><span>{label}</span><span>{i+1}</span><span>—</span></div>)}</div><div className="rp-file-rule"/><small>Illustrative document</small></div>
    ...
    </div></div><p className="rp-review-note"><AlertCircle size={25}/><span>{step===4?'Only an approved record moves to the agreed destination. The action and reviewer are recorded.':example.gate}</span></p></div></div>
  </div></section>;
}
